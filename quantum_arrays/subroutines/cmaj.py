...
def CondMAJ(N, M, k, array=None, as_circ=False):
    ...
    qubits_val = list(range(k, k + value_size)) + list(
        range(k + value_size, k + value_size + encode_size)
    )
    qubits_cmaj = (
        [flip[:][0] for flip in flip_regs] + value_reg[:] + encoding_register[:]
    )
